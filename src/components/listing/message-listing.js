...
function NewMessageList() {
  ...
  const [data, setData] = useState([
    ...
    },
    {
      ...
      message: 'Invoice for the month has been submitted.',
    },
  ]);
          ...
          <div className="card-body p-0">
            <div className="table-responsive active-projects style-1">
              <div className="tbl-caption d-flex justify-content-between align-items-center">
                <h4 className="heading mb-0">New Message List</h4>
              </div>
              <table
                  ...
                  {data.length > 0 ? (
                    data.map((item, index) => (
                      <tr key={item._id || index} role="row" className={index % 2 === 0 ? 'even' : 'odd'}>
                        <td>{item.srNo || 'Not Available'}</td>
                        <td>{item.fromDate || 'Not Available'}</td>
                        <td>{item.toDate || 'Not Available'}</td>
